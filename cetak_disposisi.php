    ...
    if(empty($_SESSION['admin'])){
        ...
        <!-- Container START -->
            ...
            <div id="lead">
                <p>Kepala Dinas</p>
                <div style="height: 50px;"></div>';
                $query = mysqli_query($config, "SELECT kadis, nip FROM tbl_instansi");
                list($kadis,$nip) = mysqli_fetch_array($query);
                if(!empty($kadis)){
                    echo '<p class="lead">'.$kadis.'</p>';
                } else {
                    echo '<p class="lead">Ir.Florentinus Anum,M.Si</p>';
                }
                if(!empty($nip)){
                    echo '<p>NIP. '.$nip.'</p>';
                } else {
